...
* You can turn on allowing mods to know if you've pressed certain keys on your keyboard, which is off by default. This is only available if runtime mod integration is on.
* You can turn off notifications in game when a mod is monitoring your keystrokes, which is on by default, but only available when keystroke monitoring is on.
* You can turn on PlumbBuddy showing you in pack seelector how much your selected packs cost, if bought at full price from EA at current prices in US dollars. This is off by default.
...
![image](/img/PB-PB-settings.png "An image of the PlumbBuddy app settings on their defaults")
